...
function retrieve_mssg(buyer_id,room_id) {
    // console.log('order_id: ', order_id, 'room_id: ', room_id)
    return(
        NeonDB.then((pool) => 
            pool.query(`SELECT * FROM messages WHERE sender_id = '${buyer_id}' AND room_id = '${room_id}'`)
            .then(result => (result.rows))
            .catch(err => console.log(err))
            // .finally(() => pool.end())
