...
import { CheckPwdResetToken, GetSeller, ResetPwd, updatePwd } from '../../api/buyer';
...
const BuyerPasswordReset = () => {
    ...
    useEffect(() => {
        GetSeller(window.localStorage.getItem('CE_buyer_id'))
        .then((result) => {
            setUserData(result)
            console.log(result)
        })
    ...
    useEffect(() => {
        CheckPwdResetToken(userData.buyer_id,location.pathname.split('/').splice(-1)[0])
        .then((result) => {
            if(result){
    ...
    function ResetPwd(params) {
        updatePwd(userData.buyer_id,pwd)
        .then((result) => {
